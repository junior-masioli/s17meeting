...
class MeetingController extends ControllerBase
{
    public function index()
    {
        ...
        $form['form'] = $this->formBuilder()->getForm(
            'Drupal\meeting\Form\Meeting\MeetingFilterForm'
        );
        // render table
        $form['table'] = [
            '#type' => 'table',
    ...
    public function tableRows($header)
    {
        ...
        if (\Drupal::request()->query->get('search')) {
            $orGroup = $query
                ...
                ->condition(
                    'm.id',
                    trim(\Drupal::request()->query->get('search'))
                )
                ->condition(
                    'm.meeting_name',
                    trim(\Drupal::request()->query->get('search'))
        ...
        foreach ($results as $data) {
            ...
            $row['uuid']['data'] = [
                ...
                '#template' =>
                    '<div class="block-filter-text-source">{{ label }}</div>',
                '#context' => [
                    'label' => $data->uuid,
                ],
            ];
            ...
            $links['edit'] = [
                'title' => $this->t('Edit'),
                'url' => Url::fromRoute('meeting.edit_form', [
                    'id' => $data->id,
                ]),
            ];
            ...
            $links['delete'] = [
                'title' => $this->t('Delete'),
                'url' => Url::fromRoute('meeting.delete_form', [
                    'id' => $data->id,
                ]),
            ];
